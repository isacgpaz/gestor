...
import { adminBottomBar, customerBottomBar } from "@/constants/bottom-bar"
...
export function BottomBar({ user }: BottomBarProps) {
  const bottomBarItems = user?.role === UserRole.ADMIN ? adminBottomBar : customerBottomBar

  return (
    <ul className="fixed bottom-0 left-0 right-0 z-10 bg-white p-3 border-t flex items-center justify-evenly gap-4">
