...
function ScheduleListItem({ schedule }: { schedule: Schedule }) {
  ...
  const { mutate: markAsReady, isPending: isMarkAsReadyPending } =
    useMarkScheduleAsReady();

  const { mutate: markAsFinished, isPending: isMarkAsFinishedPending } =
    ...
    <Card className="p-0">
      <CardHeader className="flex flex-row items-start justify-between gap-4 p-3">
        <div className="flex flex-wrap gap-2 flex-1">
          <CardTitle className="text-base">{schedule.contact.name}</CardTitle>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent
            side="bottom"
            align="end"
            className="z-10 bg-white shadow rounded border"
          >
            <DropdownMenuItem>
          ...
          <li className="flex items-center mt-1 text-sm">
            ...
            <span className="flex gap-1">
              ...
              <span className="text-slate-500 flex items-center gap-1">
                {dayjs(schedule?.startDate).format("HH:mm")} à{" "}
            ...
            <span>
              ...
              <span className="text-slate-500">
                ...
                {schedule?.kidsAmmount
                  ? ` e ${schedule?.kidsAmmount} criança(s)`
                  : null}
              </span>
            </span>
          </li>
              ...
              markAsFinished(schedule.id, {
                onSuccess() {
                  ...
                  toast({
                    ...
                    description:
                      "O horário da reserva agora está disponível para um novo agendamento.",
                    variant: "success",
                  });
                },
              })
...
function ScheduleTabs({
  ...
  useEffect(() => {
    if (isSchedulesSuccess) {
      const schedulesResults =
        schedulesResponse?.pages.map((page) => page.result).flat() ?? [];
  ...
  function scheduleAlreadyExists(schedule: Schedule, schedules: Schedule[]) {
...
export function Schedules({
  ...
  return (
    <div className="flex flex-col flex-1 mt-6 w-full">
      ...
      <DatePicker date={date} setDate={setDate} label="Selecionar data" />
